    ...
    <Box sx={{ background: 'linear-gradient(180deg, rgba(247,252,249,0.15) 0%, rgba(244,251,246,0.15) 100%)', minHeight: '100vh' }}>
      ...
      >
        <Box sx={{ position: 'relative', zIndex: 1, maxWidth: 900, mx: 'auto' }}>
          <Typography variant="h2" sx={{ fontWeight: 900, mb: 3, color: '66FFE0', textShadow: '0 2px 6px rgba(0,0,0,0.6)' }}>
            World Cup Fantasy League
          </Typography>
          <Typography variant="h6" sx={{ opacity: 0.95, maxWidth: 700, mx: 'auto', mb: 4, color: '#FCEBB6', textShadow: '0 1px 4px rgba(0,0,0,0.5)' }}>
          ...
          <Button
            ...
            sx={{ px: 6, py: 1.8, fontSize: '1rem', fontWeight: 700, borderRadius: 4 }}
          >
            Pick Your Teams
          </Button>
        </Box>
      ...
      <Container sx={{ py: { xs: 6, md: 8 } }}>
        <Box sx={{ textAlign: 'center', mb: 5, px: { xs: 0, md: 8 } }}>
          <Typography variant="h4" sx={{ fontWeight: 800, mb: 1 }}>Four picks. Endless excitement.</Typography>
        ...
        <Paper elevation={3} sx={{ mt: 5, borderRadius: 5, p: { xs: 2, md: 4 }, background: '#ffffffcc' }}>
          <AppBar position="static" elevation={0} sx={{ borderRadius: 5, bgcolor: '#7FC8A9', color: '#ffffff' }}>
          ...
          <Fade in={tab === 0} timeout={600} unmountOnExit><Box sx={{ py: 3 }}><GroupTable teams={teams} /></Box></Fade>
          <Fade in={tab === 1} timeout={600} unmountOnExit><Box sx={{ py: 3 }}><CategoryList teams={teams} /></Box></Fade>
          <Fade in={tab === 2} timeout={600} unmountOnExit><Box sx={{ py: 3 }}><FixturesPage /></Box></Fade>
          <Fade in={tab === 3} timeout={600} unmountOnExit><Box sx={{ py: 3 }}><StandingsPage /></Box></Fade>
        </Paper>
        ...
        <Paper elevation={0} sx={{ mt: 6, p: 4, borderRadius: 5, background: 'linear-gradient(135deg, #7FC8A9 0%, #B9F0D6 100%)', color: 'white', display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', justifyContent: 'space-between', gap: 3 }}>
          ...
          <Button
            ...
            size="large"
            onClick={handlePickTeams}
            sx={{ px: 5, py: 1.8, borderRadius: 4, fontWeight: 700 }}
          >
            Join the League
          </Button>
        </Paper>
